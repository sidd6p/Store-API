...
def create_app(db_url=None):
    ...
    def needs_fresh_token_callback(jwt_header, jwt_payload):
        return (
            jsonify(
                {"description": "Need fresh token", "error": "fresh_token_required"}
            ),
            401,
        )
    ...
    def revoked_token_callback(jwt_header, jwt_payload):
        return (
            jsonify(
                {"description": "The token has been expired", "error": "token_revoked"}
            ),
            401,
        )
    ...
    def missing_token_callback(error):
        return (
            jsonify({"message": "No access token", "error": "authorization_required"}),
            401,
        )
